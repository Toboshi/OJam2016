...
public class Easing
{
    ...
    public delegate void VectorTweenCallback(Vector3 i);
    ...
    public static IEnumerator tween_cr(Type easeType, Vector3 start, Vector3 end, float time, VectorTweenCallback callback)
    {
        float t = 0;
        while (t < time)
        {
            Vector3 val = Ease(easeType, start, end, t / time);
            if (callback != null) { callback(val); }
    ...
    public static Coroutine StartVectorTween(MonoBehaviour parent, Type easeType, Vector3 start, Vector3 end, float time, VectorTweenCallback callback)
    {
        return parent.StartCoroutine(tween_cr(easeType, start, end, time, callback));
    }
    ...
    public static Vector3 Ease(Type ease, Vector3 start, Vector3 end, float value)
    {
        switch (ease)
        {
    ...
    public static Vector3 EaseInOutBounce(Vector3 start, Vector3 end, float value)
    {
        ...
        float d = 1f;
        if (value < d / 2) return EaseInBounce(Vector3.zero, end, value * 2) * 0.5f + start;
        else return EaseOutBounce(Vector3.zero, end, value * 2 - d) * 0.5f + end * 0.5f + start;
    }
    ...
    public static Vector3 EaseInElastic(Vector3 start, Vector3 end, float value)
    {
        ...
        if (a == Vector3.zero || a.magnitude < Mathf.Abs(end.magnitude))
        {
            a = end;
            s = p / 4;
        }
        ...
        {
            s = p / (2 * Mathf.PI) * Mathf.Asin(end.magnitude / a.magnitude);
        }
